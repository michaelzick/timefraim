    ...
    <Card>
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          ...
          <h2 className="mt-1 text-xl font-semibold text-white">Task end pop-ups</h2>
        </div>
      </div>
      <label className="flex cursor-pointer items-start gap-3 rounded-[24px] border border-white/10 bg-white/4 px-4 py-4 text-sm text-white hover:bg-white/6">
        <input
          type="checkbox"
          checked={enabled}
          onChange={(event) => {
          ...
          disabled={!supported}
          className="mt-0.5 h-4 w-4 shrink-0 rounded border-white/20 bg-transparent accent-[var(--accent)] disabled:cursor-not-allowed disabled:opacity-60"
        />
        <span className="space-y-1">
          <span className="block font-medium text-white">Show browser pop-ups when a task ends</span>
